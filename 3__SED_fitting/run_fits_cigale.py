...
if __name__ == "__main__":
    ...
    def calc_eff_wav(wave, trans):
        tr = trans.copy().astype(float)
        wl = wave.copy().astype(float)
        np.nan_to_num(tr, copy=False)
        np.nan_to_num(wl, copy=False)
        tr *= wl
    ...
    if config["fitting"].get("run_bagpipes", False):
        ...
        pipes_params = sed_utils.correct_pipes_params(config["fitting.bagpipes"])

    if config["fitting"].get("run_cigale", False):
        cigale_params = sed_utils.correct_pipes_params(config["fitting.cigale"])
    ...
    for field in fields:
        ...
        if mpi_rank == 0:
            ...
            # try:
            sed_utils.prepare_pipes_catalogues(
                config,
                passage_dir,
                filt_dir,
                fit_ver=fit_ver,
            ...
            prepared = True
            sed_utils.reformat_pipes_cat_to_cigale(
                config,
                passage_dir,
                filt_dir,
                fit_ver=fit_ver,
