...
# with linear evolution in time
def p23_sfr(logM, z):
    """
    Return log10(SFR) [in Msol/yr] from Eq. (15).

    Parameters
    ----------
    logM : float or array
        log10 stellar mass (log M* / Msol)
    t : float or array
        cosmic time indicator (e.g., age of Universe in Gyr)

    Returns
    -------
    logSFR : float or array
    """
...
def sfms_popesso_23(
    ...
    """
    Calculate the SFMS from Popesso+23 as a function of mass and redshift.

    Parameters
    ----------
    m_star : ArrayLike
        The stellar mass values to consider. This should be given in units
        of Msol, rather than log10(M*/Msol).
    z : float
        The redshift at which to calculate the SFMS.
    a_0, a_1, a_2, a_3 : float, optional
        The best-fit parameters of the relation from Popesso+23.

    Returns
    -------
    ArrayLike
        The SFR, in units of Msol/yr.
    """
    ...
    # a_1 : float, optional
    #     _description_, by default -0.186
    # a_2 : float, optional
    #     _description_, by default 10.86
    # a_3 : float, optional
    #     _description_, by default -0.0729
    ...
    # sfr_max = 10 ** (a_0 + a_1 * t)

    # # print(sfr_max)

    # m_0 = 10 ** (a_2 + a_3 * t)

    # sfr = sfr_max / (1 + (m_0 / m_star))

    sfr = 10 ** (
        a_0 + a_1 * t - np.log10(1 + (m_star / 10 ** (a_2 + a_3 * t)) ** (-a_4))
    )
...
def sfms_merida_26(
    m_star: np.ndarray, z: float, fit: str = "intrinsic-asymm"
) -> np.ndarray:
    """
    Calculate the SFMS from Popesso+23 as a function of mass and redshift.
    ...
    merida_tab = Table.read(
        Path(
            "/media/sharedData/python/py3.13_crds/code/external/project-2025c/src/project_2025c"
        )
        / "data"
        / "sfms_merida_26.csv",
        format="ascii.tab",
    ...
    match fit:
        case "intrinsic-symm":
            fit_str = "i-s"
        case "intrinsic-asymm":
            fit_str = "i-a"
        case "statistical":
            fit_str = "s"

    print(merida_tab)
    row = merida_tab[(z >= merida_tab["z_low"]) & (z < merida_tab["z_high"])]

    # sect = "all"
    # sect = "lm"
    # row["m_break"]-=0.4
    sfr_out = np.zeros_like(m_star)
    for sect in ["all", "lm"]:
        if sect == "all":
            idx = m_star >= row["m_break"]
        else:
            idx = m_star < row["m_break"]
        sfr_out[idx] = (
            row[f"{sect}_{fit_str}_a"] + row[f"{sect}_{fit_str}_b"] * m_star[idx]
        )
    # return 10**sfr_out
    return sfr_out
    # return 10**(row[f"{sect}_{fit_str}_a"]+row[f"{sect}_{fit_str}_b"]*m_star)
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(
        1, 2, constrained_layout=True, figsize=(plotutils.formatting.aanda_textwidth, 4)
    )

    for a in axs:
        a.set_xlabel(r"$\log_{10}\left(M_*/ \rm{M}_{\odot}\right)$")
    axs[0].set_ylabel(
        r"$ \log_{10} \left(\rm{SFR}_{100}\, /\, M_{\odot} \rm{yr}^{-1}\right)$"
    )
    # for a in axs:
    #     a.set_xlabel(r"$ \log_{10} \left(\rm{sSFR}_{100}\, /\, M_{\odot} \rm{yr}^{-1}\right)$")
    # axs[0].set_ylabel(
    #     r"Number of galaxies"
    # )
    ...
    # for i, version in enumerate(["v1.3.2"]):
        ...
        for j, field in enumerate(fields):
            ...
            # )
            sed_fit_tab = Table.read(
                passage_dir.parent / "to_upload" / version / f"SED_fits_{version}_best.fits"
            )
            ...
            try:
                sed_fit_tab = sed_fit_tab[sed_fit_tab["use_sed_flag"]]
            except:
                pass
            
            print (len(sed_fit_tab))


            # plt.hist(sed_fit_tab["nebular:logU_50"], bins=np.arange(-4.2, -0.8, 0.1))
            # plt.show()

            # axs[i].hist(sed_fit_tab["stellar_mass_50"], label=version, histtype="stepfilled", alpha=0.6, bins=np.arange(6.5,11.5,0.25), color=f"C{i}")
            # axs[i].hist(sed_fit_tab["stellar_mass_50"], histtype="step", bins=np.arange(6.5,11.5,0.25), color=f"C{i}")

            # print (np.nanmedian(sed_fit_tab["stellar_mass_50"]))
            # axs[i].axvline(np.nanmedian(sed_fit_tab["stellar_mass_50"]), color=f"C{i}")
            ...
            # if i == 0:
            #     axs[i].plot(xlims, xlims - 8.0, c="k", linestyle=":")
            # if (i == 0) & (j==0):
            if j==0:
            ...
            # axs[i].axvline(np.nanmedian(sed_fit_tab["ssfr_50"]), color=f"C{i}")

            # axs[i].hist(sed_fit_tab["ssfr_50"], label=version, histtype="stepfilled", alpha=0.6, bins=np.arange(-10,-7,0.1), color=f"C{i}")
            # axs[i].hist(sed_fit_tab["ssfr_50"], histtype="step", bins=np.arange(-10,-7,0.1), color=f"C{i}")
    # fig.legend()
    ...
    for field in fields:
        tab_path = list(
            (passage_dir / "pipes" / "cats").glob(f"{field}_fit*v1.2.0*.fits")
        )
        tab_path.sort(reverse=True)
        print(tab_path)
        sed_fit_tab_1 = Table.read(tab_path[0])
        tab_path = list(
            (passage_dir / "pipes" / "cats").glob(f"{field}_fit*1.3.2*.fits")
        )
        tab_path.sort(reverse=True)
        print(tab_path)
