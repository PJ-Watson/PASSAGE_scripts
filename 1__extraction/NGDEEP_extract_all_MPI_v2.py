...
if __name__ == "__main__":
    ...
    # The number of files to load on each process
    chunk_size = config["extraction"].get("chunk_size", 8)
    # cpu_count = 8
    flt_pad = config["grism_prep"].get("pad")
    ...
    max_size = config["extraction"].get("min_size", 450)
    ...
    # Memory usage is still too high loading all files on each CPU.
    # Split the list again based on the `chunk_size` parameter.
    ...
    if len(beams_cat) > 0:
        ...
        for chunk_i, grism_subset in enumerate(
            ...
            beams_extracted = [
                (sub_beams_dir / f"{field_name}_{obj_id:0>5}.beams.fits").is_file()
                for obj_id in beams_cat["id"]
            ]

            if all(beams_extracted):
                continue

            grp = multifit.GroupFLT(
                grism_files=grism_subset,
                catalog=f"{field_name}-ir.cat.fits",
                cpu_count=1,
                sci_extn=1,
                pad=flt_pad,
            )
            os.chdir(sub_beams_dir)
    ...
    for i, row in enumerate(fit_cat):
        ...
        try:
            ...
            pline = {
                ...
                "pixfrac": 1.0,
                "pixscale": 0.06,
                "size": int(np.clip(2 * row["beam_size"] * 0.06, a_min=3, a_max=30)),
                "wcs": None,
            }
            ...
            print(f"{mpi_rank=}: Extraction failed for {obj_id}: {e}")
